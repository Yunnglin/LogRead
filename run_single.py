...
from task.single_task import SingleTask
...
if __name__ == '__main__':
    ...
    try:
        if task_id == 0:
            ...
            executor = SingleTask(task_id, hlh_path, mqtt_path)
            executor.exec_task()
        elif task_id == 1:
            # 在回流焊上运行
            logging.info('Executing on Printer')
            executor = SingleTask(task_id, printer_path, mqtt_path)
            executor.exec_task()
    except Exception:
        logging.exception('Error occurs during executing task')
